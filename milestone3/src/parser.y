        ...
        if($2 != NULL && ($2->lexeme == ":" || $2->lexeme == ":=")){
            ...
            if($2->children.size() == 3){
                if (!type_equal($1->type, $2->children[2]->type)){
                    ...
                    if ($1->type.elems != -1 || $1->type.t == "str"){
            ...
            //DEBUG(temp_addr<<temp_name)
                for (int i = 0; i < $2->children.size(); i++){
                    if ($2->children[i]->lexeme == "()" && i == 0){
                       ...
                       if(i==0) Error::other_semantic_error("TYPE_ERROR: Invalid operation near " + $2->children[i]->lexeme[0], yylineno);   
                    }
                }
            }
            if($1->lexeme == "self" && curr_symtable_class!=NULL){
                string ss="";
